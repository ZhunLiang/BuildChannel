...
system "PYTHON ~/bin/CombineGro.py -l tempLeft.gro -r tempRightT.gro -n $channel_name -x @NewGroXYZ[1] -y @NewGroXYZ[2] -z @NewGroXYZ[3] -o output_temp1.gro";
...
if($DeletPairNum!=0){
     $tune_gro="tune.gro";
     $tune_top="tune.top";
     system "PYTHON ~/bin/DelMole.py -i $IonFile -F $FirstAtomNum -S $SecondAtomNum -n @IonPairNum[1] -d $DeletPairNum -o $tune_gro";
...
system "PYTHON ~/bin/CombineGro.py -l $channel_grofile -r IonTemp.gro -n EminTf2N_Ti3C2 -x @NewGroXYZ[1] -y @NewGroXYZ[2] -z @NewGroXYZ[3] -o $OutPutChannel";
